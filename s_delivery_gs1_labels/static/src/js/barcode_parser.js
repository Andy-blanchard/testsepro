...
var BarcodeParser = Class.extend({
    ...
    gs1_checks: function(barcode){
    	var check = false;  
        if (barcode.includes('(00)') || barcode.includes('(01)') || barcode.includes('(02)') || barcode.includes('(10)') || barcode.includes('(37)'))   
            {check = true ;}        
        return check;
     },   
    ...
    check_encoding: function(barcode, encoding) {
        ...
        } else if (encoding === 'gs1') {
        	return this.gs1_checks(barcode);
	...
	get_sscc: function(barcode){
		var index_00 = barcode.indexOf('(00)');
        var sscc_code = barcode.substring(index_00+4);
        if (sscc_code.includes('('))
            { 
            var index_new_bracket = sscc_code.indexOf('(');
            sscc_code = sscc_code.substring(0, index_new_bracket);
            }
        return sscc_code;
        
     },
     
     get_product_code: function(barcode){
		var index_product = barcode.indexOf('(01)');
        if (index_product === -1)
            {index_product = barcode.indexOf('(02)');}
        var product_code = barcode.substring(index_product+5);
        if (product_code.includes('('))
            { 
            var index_new_bracket = product_code.indexOf('(');
            product_code = product_code.substring(0, index_new_bracket);
            }
        return product_code;
        
     },
     
     get_serial_lot: function(barcode){
      ...
      get_quantity: function(barcode){
		var index_37 = barcode.indexOf('(37)');
        var quantity = barcode.substring(index_37+4);
        if (quantity.includes('('))
            { 
            var index_new_bracket = quantity.indexOf('(');
            quantity = quantity.substring(0, index_new_bracket);
            }
        return quantity;
    ...
    parse_barcode: function(barcode){
        ...
        for (var i = 0; i < rules.length; i++) {
			...
			if (    rule.encoding === 'ean13' &&
                ...
                cur_barcode = cur_barcode.substr(1,12);
            } else if (rule.encoding === 'gs1' && 
            		rule.type === 'package' && 
            		this.check_encoding(barcode,'gs1') && 
            		barcode.includes('(00)')) {
	            		var cur_barcode = this.get_sscc(barcode);
            		...
            		rule.type === 'product' && 
            		this.check_encoding(barcode,'gs1') && 
            		(barcode.includes('(01)') || barcode.includes('(02)'))) {
	            		var cur_barcode = this.get_product_code(barcode);
		                parsed_result.encoding = rules[i].encoding;
		                parsed_result.type = rules[i].type;
		                parsed_result.code = cur_barcode;
		                parsed_result.base_code = cur_barcode;
		                parsed_result.lot = this.get_serial_lot(barcode);
                		parsed_result.quantity = this.get_quantity(barcode);
                		break;
		   }
