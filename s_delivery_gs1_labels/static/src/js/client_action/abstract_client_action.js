...
var ClientAction = AbstractAction.extend({
    ...
    _isProduct: function (barcode) {
        ...
        if (parsed.type === 'weight') {
            ...
            return product;
        } else if ((parsed.encoding === 'gs1') && (parsed.type === 'product')) {
        	var product = this.productsByBarcode[parsed.base_code];
        	if (product) {
                product.qty = parseInt(parsed.quantity);
                product.lot_name = parsed.lot;               
                product.encoding = 'gs1';
                var self = this;
	         	var def = this._rpc({
		                model: 'stock.production.lot',
		                method: 'search_read',
		                domain: [['name', '=', parsed.lot]],
		                limit: 1,
		            }).then(function (result) {
	            	product.lot_id = result[0].id;		            	
	            });
            }
            return product;
        } else {
    ...
    _isPackage: function (barcode) {
        var parsed = this.barcodeParser.parse_barcode(barcode);
        if ((parsed.encoding === 'gs1') && (parsed.type === 'package')) {
        	return parsed;
        } else {
        	return false;
        }
    ...
    *
    ...
    _step_product: function (barcode, linesActions) {
        ...
        var package_gs1 = this._isPackage(barcode);
        if (product) {
        	if (product.tracking !== 'none') {
                this.currentStep = 'lot';
            }
            if (product.encoding === 'gs1')
            	{var res = this._incrementLines({'product': product, 'barcode': product.code, 'lot_name': product.lot_name, 'lot_id': product.lot_id, 'gs1': true}); } 
            else {var res = this._incrementLines({'product': product, 'barcode': barcode});}
            ...
            if (package_gs1) {
           			return self._step_package_gs1(package_gs1.code, linesActions).then(success, fail);
            } else
	            {
    ...
    _step_package_gs1: function (barcode, linesActions) {
        // search stock.quant.packe location_id child_of main location ; name barcode
        // then make a search on quants package_id chilf of barcode
        // call a `_packageMakeNewLines` methode overriden by picking and inventory or increment the existing lines
        // fill linesActions + scannedLines
    ...
    _step_lot: function (barcode, linesActions) {
        ...
        return def.then(function (lot_info) {
            ...
            var res = self._incrementLines({
                ...
                'lot_name': lot_info.product.lot_name,
                'gs1': true,
                'lot_id': lot_info.product.lot_id,
            });
            } else {
            var res = self._incrementLines({
                'product': product,
            ...
            if (res.isNewLine) {
                ...
                if (product.encoding === 'gs1') {
                linesActions.push([self.linesWidget.incrementProduct, [res.id || res.virtualId, product.qty, self.actionParams.model]]);
                } else {
